...
// Internal state for the current test run
let _testResults = [];
let _currentGroup = "";

function _startGroup(name) {
    _currentGroup = name;
}
...
function validateStats() {
    _startGroup("Stats");
    logTest("currentStats is array", Array.isArray(currentStats));

    if (!Array.isArray(currentStats)) return;
    logTest("at least one stat exists", currentStats.length > 0, `count: ${currentStats.length}`);
...
function validateSkills() {
    _startGroup("Skills");
    logTest("currentSkills is array", Array.isArray(currentSkills));

    if (!Array.isArray(currentSkills)) return;
    logTest("at least one skill exists", currentSkills.length > 0, `count: ${currentSkills.length}`);

    const statKeys = currentStats.map(s => s.key);
    let allHaveFields = true;
    let allStatRefs   = true;
    let allProfLevels = true;
    const fieldBad = [], statBad = [], profBad = [];

    currentSkills.forEach((sk, i) => {
...
function validateSavingThrows() {
    ...
    logTest("currentSavingThrows exists", typeof currentSavingThrows === "object" && currentSavingThrows !== null);

    const allKeysPresent = EXPECTED_KEYS.every(k => k in currentSavingThrows);
    const missing = EXPECTED_KEYS.filter(k => !(k in currentSavingThrows));
    logTest("all 6 stat keys present", allKeysPresent,
        allKeysPresent ? "" : `missing: ${missing.join(", ")}`);

    const nonBool = EXPECTED_KEYS.filter(k => typeof currentSavingThrows[k] !== "boolean");
...
function validateInventory() {
    ...
    logTest("currentItems is array", Array.isArray(currentItems));
    ...
    currentItems.forEach((item, i) => {
        ...
        // type-specific schema check
        let schemaOk = true;
        if (item.type === "weapon") {
            schemaOk = ("stat" in item.data) && ("damage" in item.data) && ("proficient" in item.data);
        } else if (item.type === "armor") {
            schemaOk = ("baseAC" in item.data) && ("dexCap" in item.data);
        } else if (item.type === "consumable") {
            schemaOk = "effect" in item.data;
        } else if (item.type === "misc") {
            schemaOk = "description" in item.data;
        }
...
function validateDerivedValues() {
    ...
    // Skill totals: spot-check each skill against manual recalc
    ...
    currentSkills.forEach(sk => {
        ...
        // re-derive the same way renderSkills does — if they match, logic is consistent
    ...
    // Passive Perception = 10 + wis modifier + perception prof bonus
    const percSkill   = currentSkills.find(s => s.name === "Perception");
...
// Auto-run hook — only fires when the checkbox is checked, preventing noise on every keypress
rerenderAll = (function(original) {
    return function() {
        original();
